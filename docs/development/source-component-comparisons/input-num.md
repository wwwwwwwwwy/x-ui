# inputNum 与 number-field 组件差异对比

## Summary

旧组件 `inputNum`（导出 `MNumber / InputNum`）与当前组件 `number-field` 在功能语义上直接或基本对应，但两边的实现模型不同：旧组件偏业务封装和集中式 props，当前组件偏 shadcn-vue 复合组件和 slot 组合。

## 对应关系

| 项目 | 旧组件库 | 当前组件库 |
| --- | --- | --- |
| 组件 | `inputNum` | `number-field` |
| 导出 | `MNumber / InputNum` | `NumberField 复合组件` |
| 旧源码 | `source/packages/components/inputNum` | `registry/default/number-field` |
